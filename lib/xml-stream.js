...
// The sax parser is assigned several listeners for this purpose.
function parse() {
  var self = this;
  ...
  xml.on('error', function(err) {
    // sax likes to emit many error messages, but we only care about the first (DANGEROUS, I'm just trying to match node-expat behavior so no regressions are caused by using this as a drop-in replacement)
    if (!seenError) {
      seenError = true;
      self.emit('error', err);
    }
  ...
  if (self._encoder) {
    this._stream.pipe(this._encoder).pipe(xml);
  } else {
